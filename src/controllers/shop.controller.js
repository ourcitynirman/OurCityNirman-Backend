...
export const createShop = asyncHandler(async (req, res) => {
    ...
    if (role !== "vendor") throw new ApiError(403, "Only vendors can create a shop");
...
export const requestVerification = asyncHandler(async (req, res) => {
    const { _id: userId, role } = req.user;
    if (role !== "vendor") throw new ApiError(403, "Only vendors can request verification");
...
export const updateShop = asyncHandler(async (req, res) => {
    ...
    if (!["vendor", "admin"].includes(role)) throw new ApiError(403, "Access denied");
...
export const verifyShop = asyncHandler(async (req, res) => {
    if (req.user.role !== "admin") throw new ApiError(403, "Only admins can verify shops");
